...
#include "/home/jandrea/WORK/TTbarDilept_Macro/BTagSFNotReshaping/BTagSFUtil.h"

#include "/home/jandrea/WORK/TTbarDilept_Macro/MassAMWT/interface/AMWT.h"
...
public :
   ...
   TString determineChannel(int leptflav1, int leptflav2);
   
   //void writeHisto();
   
   std::vector<TH1F*> histo_list_mm;
   std::map<std::string, int> histo_map_mm;
   
   std::vector<TH1F*> histo_list_em;
   std::map<std::string, int> histo_map_em;
   
   std::vector<TH1F*> histo_list_ee;
   std::map<std::string, int> histo_map_ee;
  ...
  BTagSFUtil *fBTagSF;
   //------------------------------------------- 
   //For ttbar reconstruction in dilepton
   //--------------------------------------------
...
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("../InputFiles/proof_merged.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("../InputFiles/proof_merged.root");
      }
