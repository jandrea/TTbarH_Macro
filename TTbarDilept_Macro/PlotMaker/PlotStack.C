...
bool PlotStack(TString varname, TString namechan, TString selection, bool setlogy,
  ...
  for(unsigned int ichan = 0; ichan < channel_list.size() ; ichan++){
    ...
    for(unsigned int idatasampl = 0; idatasampl< dataSample_list.size() ; idatasampl++){
      ...
      TString histo_Data_name   = varname+channel_list[ichan]+selection+ "__"+dataSample_list[idatasampl];
    ...
    for(unsigned int imc = 0; imc < mcSample_list.size(); imc++){
      ...
      TString histo_mc_name   = varname+channel_list[ichan]+selection+ "__"+ mcSample_list[imc];
      ...
      if(channel_list[ichan] == "mumu" ) numchan = 0;
      if(channel_list[ichan] == "emu"   ) numchan = 1;
      if(channel_list[ichan] == "ee"    ) numchan = 2;
    ...
    for(unsigned int idd = 0; idd < dataDrivenTemplates_list.size(); idd++){
      ...
      TString histo_dd_name   = varname+channel_list[ichan]+selection+ "__"+ dataDrivenTemplates_list[idd];
    ...
    for(unsigned int isign = 0; isign < signalSample_list.size(); isign++){
      TString histo_mc_name   = varname+channel_list[ichan]+selection+ "_"+ signalSample_list[isign];
      TH1F * histo_tmp = (TH1F*)filechannel->Get(histo_mc_name);
      if(histo_tmp == 0)  cout << "  no existing histo with name  " << histo_mc_name << endl;
      if(niter_chan == 0){
  ...
  if (sumChannels )    info_data = "ee, #mu#mu, e#mu channels";
  ...
  TLegend* qw = 0;
  if(varname == "DeltaPhiLL" || varname == "DeltaPhiLLATLAS") qw = new TLegend(.80,.35,.95,.65);
  ...
  for(unsigned int i=0; i<mcSample_list.size(); i++){
    ...
    if(mcSample_list[i] == "TT_ljets"  ) qw->AddEntry( histo_mcSamples[1],	"t#bar{t} bkg"	 ,"f");
    if(mcSample_list[i] == "Zjets"     ) qw->AddEntry( histo_mcSamples[2],	"DY"		 ,"f");
    if(mcSample_list[i] == "TbarsChan" ) qw->AddEntry( histo_mcSamples[3],	"single top"	 ,"f");
  ...
  else if(varname == "DeltaPhiLL")   histo_ratio_data->GetXaxis()->SetTitle("Delta#phi(ll)");
  else if(varname == "DeltaPhiLLATLAS")   histo_ratio_data->GetXaxis()->SetTitle("Delta#phi(ll)/#pi");
  ...
  TString outputname = "plots/"+varname+namechan+selection+end_name;
