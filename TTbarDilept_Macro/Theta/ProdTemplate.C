...
void ProdTemplate(TString inputdistrib, std::vector<TString> sampleList, std::vector<TString> stytList, TString intputfilename, TString outputfilename){
  ...
  TH1F * distrib__DATA  	   = (TH1F*)inputfile->Get( (inputdistrib+"__DataMuEG").Data() )->Clone();
  ...
  TFile * outputfile = new TFile("theta_histofiles.root" , "recreate" );
  outputfile->cd();
...
void ProdTemplate(){
  ...
  ProdTemplate("NJetsNBJetsemuafterZsel", sampleList,  systlist,  "../TreeReader/outputroot/histofile_merged.root", "test1");
  //ProdTemplate("NJetsNBJetsemuafterZsel", sampleList,  systlist,  "../TreeReader/histofile_merged_bck.root", "test1");
  
  //ProdTemplate("NJetsNBJetsSup3JetemuafterZsel", sampleList,  systlist,  "../TreeReader/outputroot/histofile_merged.root", "test1");
